    ...
    return new Promise((resolve, reject) => {
        ...
        const onData = (data) => {
            try {
                ...
                if (expectedTotal && Object.keys(packages).length === expectedTotal && !resolved) {
                    resolved = true
                    cleanup()
                    resolve(packages)
                }
...
export async function case40_PhaseAngleDetailedQuery() {
    try {
        ...
        const isStableResponse = (responses) => {
            ...
            const impedanceVariation =
                impedanceMax > 0 ? (impedanceMax - impedanceMin) / impedanceMax : 0
            ...
            for (let attempt = 1; attempt <= 50; attempt++) {
                try {
                    ...
                    if (responseData.length === 13) {
                        // Specific length for 50 kHz 4-electrode response
        ...
        Object.entries(phaseAngleResults).forEach(([mode, results]) => {
            ...
            if (results.meaningfulResponses.length > 0) {
                ...
                console.log(
                    `   Average: ${(phaseAngles.reduce((a, b) => a + b, 0) / phaseAngles.length).toFixed(1)}°`
                )
                ...
                console.log(
                    `   Average: ${(impedanceValues.reduce((a, b) => a + b, 0) / impedanceValues.length).toFixed(1)} Ω`
                )
...
export function parse4Electrode50kHzImpedance(data) {
    ...
    console.log(
        `Phase Angle Bytes: 0x${data[6].toString(16).padStart(2, "0")} 0x${data[7].toString(16).padStart(2, "0")}`
    )
    console.log(
        `Impedance Bytes: 0x${data[8].toString(16).padStart(2, "0")} 0x${data[9].toString(16).padStart(2, "0")} 0x${data[10].toString(16).padStart(2, "0")} 0x${data[11].toString(16).padStart(2, "0")}`
    )
...
export async function case41_WeightMeasurement() {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onData = (data) => {
            ...
            const weight = wA > 5 && wA < 300 ? wA : wB > 5 && wB < 300 ? wB : null
