...
export function handleWeightStatus(statusCode, weight = null) {
    ...
    if (!errorInfo) {
        console.log(`Unknown weight status code: 0x${statusCode.toString(16)}`)
        return
    }
        ...
        biaPort.on("data", (data) => {
            ...
            if (data[0] === 0xaa && (data[2] === 0xd1 || data[2] === 0xd2)) {
                try {
                    ...
                    const parseFunc =
                        data[2] === 0xd1 ? parseLegsBodyComposition : parseArmsBodyComposition
                    ...
                    if (parsedComp) {
                        ...
                        // Send to Frontend:
                        const eventName =
                            data[2] === 0xd1 ? EVENTS.LEGS_COMP_COMPLETE : EVENTS.ARMS_COMP_COMPLETE
...
function emitHeightStatus(statusCode, height = null) {
    ...
    eventBus.emit(normalizedAction === "ABORT" ? EVENTS.HEIGHT_ERROR : EVENTS.HEIGHT_STATUS, {
        source: "HEIGHT",
        ...info,
...
function emitImpedanceStatus(statusCode, meta = {}) {
    ...
    eventBus.emit(info.action === "ABORT" ? EVENTS.IMPEDANCE_ERROR : EVENTS.IMPEDANCE_STATUS, {
        source: "IMPEDANCE",
        ...info,
...
export async function case38_20kHzImpedanceQuery() {
    ...
    try {
        ...
        const query20kHzCommand = [0x55, 0x05, 0xb1, 0x01, 0xf4]
        // Track results
        const results = {
            totalAttempts: 50,
            meaningfulResponses: [],
        ...
        for (let attempt = 1; attempt <= 25; attempt++) {
            try {
                ...
                try {
                    ...
                    console.log("error in emitting impedance status", error)
                }
                    ...
                    if (responseData[0] === 0xaa && responseData[2] === 0xb1) {
                        ...
                        if (statusResult.decision.shouldAccept) {
                            ...
                            if (isMeaningful100kHzResult(parsedResult)) {
                                ...
                                console.log(
                                    `         Right Hand: ${parsedResult.segments.rightHand}Ω`
                                )
                                console.log(
                                    `         Left Hand: ${parsedResult.segments.leftHand}Ω`
                                )
                                console.log(`         Trunk: ${parsedResult.segments.trunk}Ω`)
                                console.log(
                                    `         Right Foot: ${parsedResult.segments.rightFoot}Ω`
                                )
                                console.log(
                                    `         Left Foot: ${parsedResult.segments.leftFoot}Ω`
                                )
                                ...
                                console.log(
                                    `      ⚠️  Status success but measurement not meaningful`
                                )
                                results.zeroResponses.push({
                                    attempt,
                                    rawResponse: responseData,
...
async function collectStableRawReading(actionName = "Calibration") {
        ...
        try {
            ...
            if (data && data.length >= 14 && data[0] === 0xaa && data[2] === 0xa1) {
                ...
                if (upperNibble !== 0x10 && upperNibble !== 0x50) {
                    console.log(
                        `[CAL] ${actionName} attempt ${attempt}/${MAX_ATTEMPTS}: skipped — status 0x${statusByte.toString(16).toUpperCase()} (not stable yet)`
                    )
                    await new Promise((r) => setTimeout(r, 400))
                    continue
                }
                ...
                if (!isNaN(raw) && raw >= 0) {
                    ...
                    console.log(
                        `[CAL] ${actionName} sample ${rawReadings.length}: ${raw.toFixed(2)} kg (raw) status=0x${statusByte.toString(16).toUpperCase()}`
                    )
                    ...
                    if (rawReadings.length >= 4) {
                        ...
                        if (range < 0.1) {
                            ...
                            console.log(
                                `[CAL] ✅ ${actionName}: Stable at ${stableAvg.toFixed(4)} kg (range: ${range.toFixed(3)}) after ${attempt} attempts`
                            )
                            return stableAvg
                        }
                    }
    ...
    if (rawReadings.length === 0) {
        throw new Error(
            `${actionName} failed — scale never reached stable state (0x1x or 0x5x status) within ${MAX_ATTEMPTS} attempts`
        )
    }
    ...
    console.warn(
        `[CAL] ⚠️ ${actionName}: range never reached < 0.1 within ${MAX_ATTEMPTS} attempts. Using avg of last ${fallbackSamples.length} stable samples: ${fallbackAvg.toFixed(4)}`
    )
    return fallbackAvg
}
...
export async function performFullCalibration(knownWeightKg) {
    ...
    if (netRaw <= 0) {
        throw new Error(
            `Net raw reading (${netRaw.toFixed(3)}) is zero or negative — ensure scale is not empty and tare was run first`
        )
    }
...
export async function performMultiPointCalibration(knownWeightKg) {
    ...
    console.log(
        `[CAL] Point ${knownWeightKg} kg: rawAvg=${avgRaw.toFixed(4)}, netRaw=${netRaw.toFixed(4)}, factor=${factor.toFixed(6)}`
    )
