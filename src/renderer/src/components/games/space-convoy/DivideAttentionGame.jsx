...
const ARENA = {
    ...
    t: HUD_H + PAD,
    r: CW - PAD,
    b: CH - PAD - SUBMIT_BTN.h - SUBMIT_BTN.bottomPad - 20
}
...
function buildBackground() {
    ...
    for (let i = 0; i < 560; i++) {
        ...
        ctx.fillStyle = `hsla(${hue},${sat}%,${lit}%,${alp})`
        ctx.fill()
        if (size > 2.4) {
            const fa = alp * 0.28,
                len = size * 3.2
            ctx.strokeStyle = `hsla(${hue},${sat}%,96%,${fa})`
            ctx.lineWidth = 0.55
            ctx.beginPath()
            ctx.moveTo(x - len, y)
            ctx.lineTo(x + len, y)
...
function physics(ps, dt, moving, onCollision = null) {
    ...
    for (const p of ps) {
        if (p.x - p.radius < ARENA.l) {
            p.x = ARENA.l + p.radius
            p.vx = Math.abs(p.vx)
            p.baseVx = Math.abs(p.baseVx)
        }
        if (p.x + p.radius > ARENA.r) {
            p.x = ARENA.r - p.radius
            p.vx = -Math.abs(p.vx)
            p.baseVx = -Math.abs(p.baseVx)
        }
        if (p.y - p.radius < ARENA.t) {
            p.y = ARENA.t + p.radius
            p.vy = Math.abs(p.vy)
            p.baseVy = Math.abs(p.baseVy)
        }
        if (p.y + p.radius > ARENA.b) {
            p.y = ARENA.b - p.radius
            p.vy = -Math.abs(p.vy)
            p.baseVy = -Math.abs(p.baseVy)
        }
    }
...
function drawTrail(ctx, p, tintColor) {
    ...
    for (let i = 1; i < p.trail.length; i++) {
        ...
        ctx.strokeStyle = tintColor
            ...
            : p.isTarget
              ? `rgba(61,179,158,${t * 0.24})`
              : `rgba(180,160,255,${t * 0.15})`
        ctx.lineWidth = p.radius * 0.24 * t
        ctx.lineCap = "round"
        ctx.stroke()
    }
}
...
function spawnPopup(pool, x, y, points, tierPrimary) {
    pool.push({
        ...
        color: points > 0 ? tierPrimary || "#7FFFD4" : "#FF5252",
        life: 1.0,
        vy: -2.5
    })
}
...
function updateAndDrawPopups(ctx, pool, dt) {
    ...
    for (let i = pool.length - 1; i >= 0; i--) {
        ...
        p.y += p.vy * step
        ctx.save()
        ctx.globalAlpha = easeOutExp(p.life)
        ctx.font = `bold 52px 'Arial Black', Arial, sans-serif`
        ctx.fillStyle = "rgba(0,0,0,0.45)"
        ctx.fillText(p.label, p.x + 2.5, p.y + 2.5)
...
function drawCountIn(ctx, ci, tier) {
    ...
    ctx.beginPath()
    ctx.roundRect(-pw / 2, -ph / 2, pw, ph, 24)
    ctx.fill()
    ...
    ps.forEach((p, idx) => {
        if (p.selected) {
            if (p.isTarget) {
                // Correct Hit
                const hasSpeedBonus =
                    p.responseTimeMs !== null && p.responseTimeMs < speedThresholdMs
...
export default function SpaceConvoy() {
    ...
    const [hud, setHud] = useState({
        ri: 0,
        totalScore: 0,
        tierPrimary: TIER_COLOURS[0].primary,
        tierGlow: TIER_COLOURS[0].glow,
        roundName: ""
    })

    useEffect(() => {
        bgRef.current = buildBackground()
    }, [])
        ...
        setTimeout(
            () =>
                navigate("/space-convoy-complete", {
                    state: {
                        results: G.current.results,
                        totalScore: apiState.current.totalScore,
                        nextRoute
                    }
                }),
            500
        ...
        const loop = (t) => {
            ...
            if (g.sess === SESS.PLAYING) {
                ...
                switch (g.ph) {
                    ...
                    case PHASE.FREEZE:
                        g.ps.forEach((p) => {
                            if (p.hitAnimStart !== null)
                                p.hitAnim = Math.min(1, (performance.now() - p.hitAnimStart) / 300)
                        })
                        physics(g.ps, dt, false)
                        break
                }
    ...
    const draw = useCallback((dt = 16.667) => {
        ...
        if (g.ph === PHASE.START || g.ph === PHASE.REVEAL) {
            g.ps.forEach((p) => {
                if (p.opacity > 0) drawTrail(ctx, p, p.isTarget ? null : "rgba(180,160,255,")
            })
        }
        ...
        g.ps.forEach((p) => {
            ...
            if ((g.ph === PHASE.OVER || g.ph === PHASE.FREEZE) && p.selected) {
                ...
                const img = glow[si] || stim[si]
                if (img)
                    drawParticleImg(ctx, img, p, (img === glow[si] ? GLOW_SCALE[si] : 1) * pulse)
                ...
                if (img) drawParticleImg(ctx, img, p)
                    ...
                    ctx.fillStyle = p.isTarget ? tc.primary : "#F4B459"
                    ctx.fill()
                }
            }
            ctx.restore()
        ...
        (cx, cy) => {
            ...
            for (let i = g.ps.length - 1; i >= 0; i--) {
                ...
                if (!p.selected && d2d({ x, y }, p) <= p.radius + 8) {
                    ...
                    p.hitAnimStart = performance.now()
                    p.hitAnim = 0
                    sfx.particleTap()
                    if (p.isTarget) {
