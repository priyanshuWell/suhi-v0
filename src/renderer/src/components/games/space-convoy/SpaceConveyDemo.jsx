...
export default function SpaceConveyDemo({ activeSessionId, onComplete, handleMoveToComplete }) {
    ...
    const finishPracticeTrial = useCallback(
        async (ps, submitTimeMs) => {
            const api = apiRef.current
            // const activeSessionId = apiRef.current.activeSessionId;
                        ...
                        if (g.elapsed >= STEP_RESULT_MS && !g.resultHandled) {
                            ...
                            if (g.trial1Passed && g.practiceRound === 1) {
                                ...
                                if (g.failCount >= 3) {
                                    ...
                                    setDisplayMsg(
                                        `Keep track of the ${trialNum} set. Let's try again.`
                                    )
                                    setTimeout(() => {
                                        g.ps = spawnDemo()
                                        g.allGuessed = false
    ...
    const draw = useCallback((time) => {
        ...
        g.ps.forEach((p) => {
            ...
            if (
                p.selected &&
                (g.step === STEP.STOPPED || g.step === STEP.RESULT || g.step === STEP.DONE)
            ) {
                if (p.isTarget) {
