...
const STAGES = [
    {
        stageId: 1,
        stageCode: "GALAXY_SWIRL",
        stageName: "Galaxy Swirl",
        targets: ["BA", "ST", "BL"],
        juice: "#931621"
    },
    {
        stageId: 2,
        stageCode: "TROPICAL_TREASURE",
        stageName: "Tropical Treasure",
        targets: ["AP", "KI", "MA"],
        juice: "#B5864C"
    },
    {
        stageId: 3,
        stageCode: "SUNRISE_SPLASH",
        stageName: "Sunrise Splash",
        targets: ["BA", "ST", "OR"],
        juice: "#C8AA8F"
    },
    {
        stageId: 4,
        stageCode: "FROZEN_SKY",
        stageName: "Frozen Sky",
        targets: ["BL", "AP", "PI"],
        juice: "#F2B34C"
    },
    {
        stageId: 5,
        stageCode: "JUNGLE_JUMP",
        stageName: "Jungle Jump",
        targets: ["BA", "KI", "MA"],
        juice: "#448A5A"
    },
    {
        stageId: 6,
        stageCode: "RAINBOW_BURST",
        stageName: "Rainbow Burst ",
        targets: ["ST", "BL", "OR"],
        juice: "#7E9BD0"
    }
]
...
const JAR_CLIP_PATH =
    "polygon(12% 0%, 88% 0%, 95% 5%, 100% 22%, 100% 70%, 96% 90%, 86% 100%, 14% 100%, 4% 90%, 0% 70%, 0% 22%, 5% 5%)"
...
function computeMetrics(events) {
    ...
    const rtSd =
        rtB.length > 1
            ? Math.sqrt(rtB.reduce((a, c) => a + (c - mRt) ** 2, 0) / (rtB.length - 1))
            : 0
        ...
        constructs: {
            ...
            cognitive_flexibility: clamp01(
                b1.acc && b1.rt && accSwitch != null && rtSwitch != null
                    ? 1 - (0.5 * (accSwitch / b1.acc) + 0.5 * (rtSwitch / b1.rt))
                    : null
            ),
...
export default function SmoothieSlashGame() {
  ...
  ──────────────────────────────────────────────────────────────────── */
    ...
    const showStageBanner = useCallback(
        (sIdx) => {
    ...
    const doBlockPause = useCallback(
        (newSIdx) => {
            fruitsRef.current = []
            halvesRef.current = []
    ...
    const pushStageComplete = useCallback(async (stageId, startedAt, completedAt, slashes) => {
        ...
        try {
            ...
            console.groupCollapsed(
                `🍉 [SmoothieSlash] stage/complete REQUEST — stage_id: ${stageId}`
            )
            ...
            if (!sessionIdRef.current) {
                console.warn(
                    "[SmoothieSlash] stage/complete skipped — no game_session_id (offline mode)"
                )
                return null
            }

            const res = await apiStageComplete(
            ...
            // ── RESPONSE log ───────────────────────────────────────────────
            console.groupCollapsed(
                `✅ [SmoothieSlash] stage/complete RESPONSE — stage_id: ${stageId}`
            )
            ...
            const cr = shaped.filter(
                (s) =>
                    s.action === "NONE" &&
                    !STAGES.flatMap((st) => st.targets).includes(s.fruit_code)
            ).length
    ...
    const endGame = useCallback(async () => {
        ...
        const finalRes = await pushStageComplete(
            stageIdRef.current,
        ...
        if (sessionIdRef.current) {
            try {
                ...
                console.warn(
                    "[SmoothieSlash] game/complete failed — using local metrics:",
                    e.message
                )
            }
        }
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            if (sIdx !== eng.stage) {
                ...
                const blockSlashes = eventsRef.current.filter(
                    (e) => e.block_number === prevBlock + 1
                )
                pushStageComplete(prevStageId, prevStartedAt, completedAt, blockSlashes).then(
                    (res) => {
            ...
            if (!pausedRef.current) {
                ...
                if (p && jar) {
                    ...
                    if (now - eng.lastSpawn > SPAWN_MS && alive.length < MAX_ALIVE) {
                        ...
                        const freshPool = ALL_CODES.filter(
                            (c) => !st.targets.includes(c) && !prevTargets.includes(c)
                        )
                        ...
                        if (roll < 0.5) {
                            ...
                            pool = freshPool.length
                                ? freshPool
                                : ALL_CODES.filter((c) => !st.targets.includes(c)) // Fresh Distractor — 20%
                        } else {
                        ...
                        fruitsRef.current.push({
                            ...
                            code,
                            stageIdx: sIdx,
                            wasPrevTarget: prevTargets.includes(code),
                            x: x0,
                            y: H + 20,
                            vx: vx0,
                            vy: vy0,
                    ...
                    halvesRef.current.forEach((h) => {
                        ...
                        if (h.mode === "jar") {
                            ...
                            if (h.y >= jar.counterY && h.vy > 0 && outside) {
                                h.y = jar.counterY
                                h.rest = now
                            }
            ...
            overflowDripsRef.current = overflowDripsRef.current.filter(
                (d) => now - d.t0 < d.dur + 300
            )
            ...
            halvesRef.current.push(
                {
                    ...
                    side: "L",
                    jarOff: -12,
                    mx: motionValue(f.x - 20),
                    my: motionValue(f.y),
                    mr: motionValue(f.rot)
                },
                {
                    ...base,
                    id: uid(),
    ...
    if (!sliceSoundPools.current) {
        sliceSoundPools.current = [
            Array.from({ length: POOL_SIZE }, () => {
                const a = new Audio(cutSlice1)
                a.volume = 0.65
                return a
            }),
            Array.from({ length: POOL_SIZE }, () => {
                const a = new Audio(cutSlice2)
                a.volume = 0.65
                return a
            })
        ]
            ...
            if (correct) {
                ...
                if (eng.combo % PTS.COMBO_EVERY === 0) {
                    ...
                    popsRef.current.push({
                        ...
                        y: f.y - 54,
                        t0: now,
                        text: `${eng.combo} COMBO +${PTS.COMBO_BONUS}`,
                        kind: "combo"
                    })
                }
    ...
    const onDown = (e) => {
        ...
        trailRef.current.push({
            ...pt,
            sid: strokeRef.current?.sid,
            color: strokeRef.current?.color
        })
    }
    const onMove = (e) => {
    ...
    return (
        <div
            className="w-full h-full minfullh flex flex-col"
            style={{ background: "#101B18", fontFamily: "'Nunito',sans-serif" }}
        >
            <style>{`
      ...
      `}</style>
                ...
                <>
                    ...
                    >
                        <div
                            ...
                            style={{ background: "rgba(8,4,20,.4)", zIndex: 0 }}
                        />
                                ...
                                >
                                    ...
                                    >
                                        {INTRO_SLIDES[introStep].type === "component" ? (
                                            (() => {
                                                const SlideComp = INTRO_SLIDES[introStep].component
                                                return <SlideComp />
                                            })()
                                        ) : (
                                            <div
                                                ...
                                                style={{
                                                    background:
                                                        "linear-gradient(180deg,rgba(46,26,74,.95),rgba(24,14,42,.95))",
            ...
            {screen === SCREENS.COUNTDOWN && (
                ...
                >
                    <div
                        style={{ position: "absolute", inset: 0, background: "rgba(8,4,20,.4)" }}
                    />
                    <span
                        className="display"
                        style={{
            ...
            {screen === SCREENS.GAME && (
                ...
                >
                    ...
                    >
                        ...
                        >
                            <span className="display font-extrabold text-md text-amber-200 leading-none">
                                SCORE
                            </span>
                  ...
                  (the banner itself shows the fruits during the transition) */}
                        {!bannerAnim && (
                            ...
                            >
                                ...
                                <p
                                    className="display mt-3 tracking-wider"
                                    style={{
                                        fontSize: "1.4vmax",
                                        color: "#F0E6C8",
                  ...
                  render ON TOP of it with mix-blend-mode: multiply to tint through. */}
                        <div
                            key={pulse}
                            className="absolute inset-0"
                            style={{ animation: "jarNudge .28s ease-out", zIndex: 1 }}
                        >
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        ...
                                        WebkitMaskImage: `url(${splashCrownMask})`,
                                        maskImage: `url(${splashCrownMask})`,
    ...
    `,
                                    ...
                                    boxShadow:
                                        "inset 0 8px 14px rgba(255,255,255,.22), inset 0 -12px 22px rgba(0,0,0,.32), inset 16px 0 24px rgba(0,0,0,.18), inset -16px 0 24px rgba(0,0,0,.18)"
                                }}
                            >
                                ...
                                <div
                                    ...
                                    style={{
                                        height: "38%",
                                        backgroundImage:
                                            "radial-gradient(circle, rgba(255,255,255,.28) 1px, transparent 1.8px)",
      ...
      still read even once the jar is full, instead of the liquid just hiding it */}
                            <div
                                ...
                                style={{
                                    background:
                                        "linear-gradient(115deg, rgba(255,255,255,.55) 0%, rgba(255,255,255,0) 16%, rgba(255,255,255,0) 70%, rgba(255,255,255,.22) 100%)",
                                    mixBlendMode: "screen",
                                    opacity: 0.5
                                }}
                            />
                        ...
                        {splashLive && fill < 1 && (
                            <div
                                ...
                                style={{
                                    ...
                                    borderRadius: "50%",
                                    opacity: 0.9,
                                    animation: "jarSplash .55s ease-out forwards",
                                    zIndex: 3
                                }}
                            />
                        )}
...
@keyframes liquidWobble { 0%,100%{transform:scaleX(1) scaleY(1)} 50%{transform:scaleX(1.008) scaleY(.995)} }`}</style>
    ...
    so the browser never has to recompute layout mid-spring */}
                        ...
                        {bannerAnim && (
                            <motion.div
                                ...
                                initial={{
                                    opacity: 0,
                                    scale: 0.7,
                                    y: bannerYOffset(playRef, "center")
                                }}
                                animate={{
                                    opacity: 1,
                                ...
                                onAnimationComplete={() => {
                                    if (bannerAnim === "center") {
                                        ...
                                        setTimeout(() => {
                                            // this is a deliberate static hold, not mid-motion
                                            setBannerAnim(null)
                                            setBannerFruits([])
                    ...
                    {paused && (
                        <div
                            ...
                            style={{
                                ...
                                animation: "pauseOverlayIn .3s ease"
                            }}
                        ></div>
                    )}
                    ...
                    {fruitsRef.current.map((f) => (
                        ...
                        >
                            <img
                                src={FRUITS[f.code].img}
                                alt={FRUITS[f.code].name}
                            ...
                            return (
                                <motion.div
                                    ...
                                    style={{
                                        ...
                                        filter:
                                            h.mode === "out"
                                                ? "grayscale(.45) brightness(.9)"
                                                : "brightness(1.1)"
                                    }}
                                >
                                    <img
                                        src={FRUITS[h.code].cutImg}
                                        alt={FRUITS[h.code].name}
                                        style={{
                                            width: "100%",
                                            height: "100%",
                                            objectFit: "contain"
                   ...
                   GPU compositor path and never trigger a React layout pass              */
                            <motion.div
                                ...
                                style={{
                                    ...
                                    y: "-50%",
                                    clipPath:
                                        h.side === "L" ? "inset(0 52% 0 0)" : "inset(0 0 0 52%)",
                                    opacity: h.rest
                                        ? Math.max(0, 1 - (performance.now() - h.rest) / 600)
                                        : 1,
                                    filter:
                                        h.mode === "out"
                                            ? "grayscale(.45) brightness(.9)"
                    ...
                    {popsRef.current.map((pp) => (
                        <div
                            key={pp.id}
                            className="absolute display pointer-events-none"
                            style={{
                                zIndex: 10,
                                left: pp.x,
                                top: pp.y,
            ...
            {screen === SCREENS.REPORT && (
                ...
                >
                    ...
                    >
                        <span
                            ...
                            style={{
                                ...
                                color: "#F5C842",
                                textShadow: "0 3px 4px rgba(0,0,0,.5)"
                            }}
                        >
                            {score}
                        </span>
                    ...
                    >
                        <div
                            className="absolute"
                            style={{ left: "17%", right: "12%", top: "12%", bottom: "13%" }}
                        >
                            ...
                            <div
                                ...
                                style={{ height: 18, background: "rgba(255,255,255,.9)" }}
                            >
                                <div
                            ...
                            <div className="flex flex-col gap-3.5">
                                {STAGES.map((s) => {
                                    ...
                                    const barColor =
                                        pct >= 80 ? "#5DD62C" : pct >= 50 ? "#F5A623" : "#E8443C"
                                    return (
