...
const StepLine = ({ done }) => (
    <div
        style={{
            ...
            marginTop: "-22px",
            marginBottom: "22px",
            background: done ? "linear-gradient(90deg, #00c97a, #006640)" : "rgba(255,255,255,0.1)",
...
const ResultRow = ({ label, value }) => (
    ...
    >
        <span
            style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", fontFamily: "sans-serif" }}
        >
            {label}
        </span>
    ...
    const handleTare = async () => {
        ...
        try {
            ...
            if (result?.success) {
                ...
                setStatus({
                    type: "success",
                    text: `Zero offset captured: ${result.calibration.zeroOffset.toFixed(4)} raw`
                })
                setTimeout(() => {
                    setStatus(null)
                    setStep(3)
                ...
                setStatus({
                    type: "error",
                    text: `Tare failed: ${result?.error || "Unknown error"}`
                })
            }
        } catch (err) {
            setStatus({ type: "error", text: `Error: ${err.message}` })
    ...
    const handleCal50 = async () => {
        ...
        try {
            ...
            if (result?.success) {
                ...
                setStatus({
                    type: "success",
                    text: `50 kg point captured — factor: ${result.factor.toFixed(6)}`
                })
                setTimeout(() => {
                    setStatus(null)
                    setStep(4)
                ...
                setStatus({
                    type: "error",
                    text: `50 kg calibration failed: ${result?.error || "Unknown error"}`
                })
            }
        } catch (err) {
            setStatus({ type: "error", text: `Error: ${err.message}` })
    ...
    const handleCal100 = async () => {
        ...
        try {
            ...
            if (result?.success) {
                ...
                setStatus({
                    type: "info",
                    text: `Computing averaged factor: ${avgFactor.toFixed(6)}…`
                })
                ...
                if (applyResult?.success) {
                    ...
                    setStatus({
                        type: "success",
                        text: `Calibration complete! Avg factor: ${avgFactor.toFixed(6)}`
                    })
                    setStep(5)
                } else {
                    setStatus({
                        type: "error",
                        text: `Failed to save factor: ${applyResult?.error || "Unknown error"}`
                    })
                }
            } else {
                setStatus({
                    type: "error",
                    text: `100 kg calibration failed: ${result?.error || "Unknown error"}`
                })
            }
        } catch (err) {
            setStatus({ type: "error", text: `Error: ${err.message}` })
            ...
            >
                ...
                >
                    <div>
                        ...
                        >
                            3-point calibration (0 kg → 50 kg → 100 kg) — results averaged &amp;
                            saved permanently
                        </p>
                    </div>
                    <button
                ...
                {step === 1 && (
                    <div style={{ animation: "calFadeIn 0.3s ease" }}>
                        <InfoCard>
                            1. Make sure the <strong style={{ color: "#9ad9ff" }}>BIA scale</strong>{" "}
                            is plugged in via USB.
                            <br />
                            2. Ensure the scale is{" "}
                            <strong style={{ color: "#9ad9ff" }}>completely empty</strong> (no
                            weight on it).
                            <br />
                            3. Click <strong style={{ color: "#9ad9ff" }}>Connect</strong> to
                            proceed.
                            {biaPort && (
                                <div
                                    style={{
                ...
                {step === 2 && (
                    <div style={{ animation: "calFadeIn 0.3s ease" }}>
                        <InfoCard>
                            ...
                            <br />
                            <br />
                            <strong style={{ color: "#ff9f40" }}>
                                Remove everything from the scale.
                            </strong>
                            <br />
                            The scale must be completely empty at 0 kg.
                            <br />
                            Click <strong style={{ color: "#9ad9ff" }}>Read Zero (0 kg)</strong> to
                            capture the baseline offset.
                        </InfoCard>
                        {/* Weight label */}
                        <div
                            style={{
                ...
                {step === 3 && (
                    <div style={{ animation: "calFadeIn 0.3s ease" }}>
                        <InfoCard>
                            Zero offset captured ✅<br />
                            <br />
                            Now place a{" "}
                            <strong style={{ color: "#ff9f40" }}>50 kg reference weight</strong> on
                            the scale.
                            <br />
                            Stand still and wait for a stable reading, then click{" "}
                            <strong style={{ color: "#9ad9ff" }}>Read 50 kg</strong>.
                        </InfoCard>
                        {/* Weight label */}
                        <div
                            style={{
                ...
                {step === 4 && (
                    <div style={{ animation: "calFadeIn 0.3s ease" }}>
                        <InfoCard>
                            50 kg point captured ✅<br />
                            <br />
                            Now place a{" "}
                            <strong style={{ color: "#ff9f40" }}>100 kg reference weight</strong> on
                            the scale.
                            <br />
                            Stand still and wait for a stable reading, then click{" "}
                            <strong style={{ color: "#9ad9ff" }}>Read 100 kg</strong>.
                        </InfoCard>
                        {/* Weight label */}
                        <div
                            style={{
                        ...
                        <PrimaryBtn onClick={handleCal100} busy={busy}>
                            {busy ? (
                                <>
                                    <Spinner size={26} /> Reading 100 kg…
                                </>
                            ) : (
                                "⚖️ Read 100 kg"
                            )}
                ...
                {step === 5 && finalCal && (
                    <div style={{ animation: "calFadeIn 0.4s ease", textAlign: "center" }}>
                        ...
                        >
                            {point50 && (
                                <ResultRow label="50 kg factor" value={point50.factor.toFixed(6)} />
                            )}
                            {point100 && (
                                <ResultRow
                                    label="100 kg factor"
                                    value={point100.factor.toFixed(6)}
                                />
                            )}
                            <ResultRow label="Averaged factor" value={finalCal.factor.toFixed(6)} />
                            <ResultRow
                                label="Zero offset"
                                value={finalCal.zeroOffset.toFixed(4) + " raw"}
                            />
                            <ResultRow
                                label="Calibrated at"
                                value={new Date(finalCal.calibratedAt).toLocaleString()}
                            />
                        </div>
