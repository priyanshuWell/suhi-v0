...
const BackspaceIcon = () => (
  <svg width="24" height="18" viewBox="0 0 36 26" fill="none">
    <path d="M11 2h20a2 2 0 012 2v18a2 2 0 01-2 2H11L1 13 11 2z"
      stroke={CYAN} strokeWidth="1.8" fill="none" strokeLinejoin="round" />
    <path d="M17 9l8 8M25 9l-8 8"
      stroke={CYAN} strokeWidth="1.8" strokeLinecap="round" />
  </svg>
)
const EnterIcon = () => (
  <svg width="22" height="16" viewBox="0 0 28 20" fill="none">
    <path d="M22 3v8a3 3 0 01-3 3H5"
      stroke={CYAN} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
...
const ShiftIcon = ({ active }) => (
  <svg width="18" height="18" viewBox="0 0 22 22" fill="none"
    ...
    <path d="M11 2L2 13h5v7h8v-7h5L11 2z"
      stroke={CYAN} strokeWidth="1.8"
...
const CapsIcon = ({ active }) => (
  <svg width="18" height="18" viewBox="0 0 22 22" fill="none"
    style={{ opacity: active ? 1 : 0.7 }}>
    <path d="M11 2L2 11h5v2h8v-2h5L11 2z"
      stroke={CYAN} strokeWidth="1.8"
      fill={active ? 'rgba(70,168,203,0.18)' : 'none'}
      strokeLinejoin="round" />
    <rect x="6" y="15" width="10" height="3" rx="1"
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 8,
        border: isDown ? '1px solid rgba(70,168,203,0.4)' : '1px solid rgba(255,255,255,0.04)',
...
function CharKey({ pair, active, onPress, onRelease, shifted, height = 52 }) {
  ...
  const id = `k-${normal}`
  return (
    <Key id={id} active={active} onPress={(i) => { onPress(i); }} onRelease={onRelease}
      style={{ height, flex: 1, minWidth: 0 }}
    >
      {shifted !== undefined && (
        <span style={{ color: '#4a5568', fontSize: 10, lineHeight: 1, marginTop: 4, alignSelf: 'flex-start', paddingLeft: 6 }}>
          {shift}
        </span>
      )}
      <span style={{ color: CYAN, fontSize: 18, fontWeight: 400, lineHeight: 1, marginBottom: 4 }}>
        {display}
      </span>
    </Key>
  )
...
function FullKeyboard({ onKeyPress, onBackspace, onSubmit, onClose }) {
  ...
  const row = (pairs, extra = {}) => pairs.map((pair) => (
    <Key key={pair[0]} id={`k-${pair[0]}`} active={active}
      onPress={(id) => { press(id); emit(pair) }}
      onRelease={release}
      style={{ height: h, flex: 1, minWidth: 0, ...extra }}
    >
      <span style={{ color: '#4a5568', fontSize: 9.5, lineHeight: 1, marginTop: 3, alignSelf: 'flex-start', paddingLeft: 5 }}>
        {pair[1]}
      </span>
      <span style={{ color: CYAN, fontSize: 17, fontWeight: 400, lineHeight: 1, marginBottom: 3 }}>
        {upper ? pair[1] : pair[0]}
      </span>
    </Key>
  ))

  const modStyle = { background: MOD_BG, border: '1px solid rgba(255,255,255,0.06)', boxShadow: 'none' }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        ...
        <Key id="caps" active={active}
          ...
          style={{
            ...
            border: caps ? '1px solid rgba(70,168,203,0.3)' : '1px solid rgba(255,255,255,0.06)',
            boxShadow: caps ? '0 0 8px rgba(70,168,203,0.25)' : 'none',
          }}>
          <CapsIcon active={caps} />
          <span style={{ color: caps ? CYAN : GREY, fontSize: 9, marginTop: 1 }}>CAPS</span>
        </Key>
        {row(ROW2)}
        {/* Enter */}
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        {/* Left Shift */}
        <Key id="shiftL" active={active}
          onPress={() => { press('shiftL'); setShift(s => !s) }} onRelease={release}
          style={{
            height: h, flex: 2.4,
            background: shift ? 'rgba(70,168,203,0.1)' : MOD_BG,
            border: shift ? '1px solid rgba(70,168,203,0.3)' : '1px solid rgba(255,255,255,0.06)',
            boxShadow: shift ? '0 0 8px rgba(70,168,203,0.25)' : 'none',
          }}>
          <ShiftIcon active={shift} />
          <span style={{ color: shift ? CYAN : GREY, fontSize: 9, marginTop: 1 }}>Shift</span>
        </Key>
        {row(ROW3)}
        {/* Right Shift */}
        <Key id="shiftR" active={active}
          onPress={() => { press('shiftR'); setShift(s => !s) }} onRelease={release}
          style={{
            height: h, flex: 2.8,
            background: shift ? 'rgba(70,168,203,0.1)' : MOD_BG,
            border: shift ? '1px solid rgba(70,168,203,0.3)' : '1px solid rgba(255,255,255,0.06)',
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        {/* Ctrl */}
        <Key id="ctrl" active={active} onPress={press} onRelease={release}
          style={{ height: h, flex: 1.2, ...modStyle }}>
          <span style={{ color: GREY, fontSize: 11 }}>Ctrl</span>
        </Key>
        {/* Alt */}
        <Key id="altL" active={active} onPress={press} onRelease={release}
        ...
        {/* Alt Gr */}
        <Key id="altR" active={active} onPress={press} onRelease={release}
          style={{ height: h, flex: 1, ...modStyle }}>
          <span style={{ color: GREY, fontSize: 10 }}>AltGr</span>
        </Key>
        {/* Ctrl */}
        <Key id="ctrlR" active={active} onPress={press} onRelease={release}
          style={{ height: h, flex: 1.2, ...modStyle }}>
          <span style={{ color: GREY, fontSize: 11 }}>Ctrl</span>
        </Key>
      </div>
