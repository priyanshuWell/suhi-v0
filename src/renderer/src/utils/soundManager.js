...
const SOUND_VOLUMES = {
    round_start: 0.35,
    freeze: 0.4,
    particle_tap: 0.3, // same volume for hit and false alarm
...
function playNoise(
    ctx,
    mg,
    { duration = 0.12, freq = 800, q = 1.5, volume = 0.4, type = "bandpass" } = {}
) {
...
function playTone(
    ctx,
    mg,
    { freq = 440, type = "sine", duration = 0.2, volume = 0.3, attack = 0.01, release = 0.15 } = {}
) {
...
export class SoundManager {
        ...
        await Promise.all(
            Object.entries(SOUND_FILES).map(async ([name, url]) => {
                ...
                try {
                    ...
                    console.warn(
                        `[SoundManager] "${name}" failed to load, using synth fallback.`,
                        e
                    )
                }
            })
    ...
    freeze() {
        ...
        playNoise(this._ctx, this._master, {
            duration: 0.08,
            freq: 200,
            q: 0.8,
            volume: 0.25,
