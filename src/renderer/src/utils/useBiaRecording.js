...
export function useBIARecording({ sessionId, userId, screeningSessionId, bufferType = "BIA" }) {
                ...
                recorder.onstop = async () => {
                    ...
                    try {
                        ...
                        const localSavePromise = (async () => {
                            try {
                                ...
                                if (localResult?.success) {
                                    console.log(
                                        `[BIA REC] 💾 Saved locally → ${localResult.filePath}`
                                    )
                                } else {
                                    console.warn(
                                        "[BIA REC] ⚠️ Local save failed:",
                                        localResult?.error
                                    )
                                }
                            } catch (localErr) {
                        ...
                        if (screeningSessionId && shmPath) {
                            ...
                            console.log(
                                `[BIA REC] ✅ Buffer collection DONE — role: "${role}"`,
                                bufferResult
                            )
                        }
                        ...
                        console.error(
                            `[BIA REC] ❌ Upload/save FAILED — role: "${role}"`,
                            err.message
                        )
                        resolve(null)
                    }
    ...
    const stopAndSend = useCallback(() => {
        console.log(
            "[BIA REC] 🏁 stopAndSend() — BIA completed successfully, sending full recording"
        )
