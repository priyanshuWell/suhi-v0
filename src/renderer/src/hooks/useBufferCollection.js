...
export const usePageBufferCollection = ({
  ...
  const _processAndSendBuffer = useCallback(async (reason = 'duration') => {
    if (hasCollectedRef.current) return;
    if (!screeningSessionId || !userId || !kioskId) {
      console.warn('[BUFFER COLLECTION] Missing required params — screeningSessionId:', screeningSessionId, 'userId:', userId, 'kioskId:', kioskId);
      return;
    }
    ...
    console.log(`[BUFFER COLLECTION] Duration recorded: ${(elapsed / 1000).toFixed(1)}s, bufferType: ${bufferType}`);
    ...
    try {
      ...
      console.log(`[BUFFER COLLECTION] Buffer size: ${(blob.size / 1024).toFixed(1)}KB, chunks: ${chunksRef.current.length}`);
      ...
      console.error('[BUFFER COLLECTION] ❌ Error during buffer collection:', collectionError.message);
  ...
  const startCollection = useCallback(async () => {
    ...
    try {
      ...
      console.log('[BUFFER COLLECTION] Recording started — will collect for max', maxDuration / 1000, 'seconds');
  ...
  }, [isEnabled, screeningSessionId, userId, kioskId, bufferType, maxDuration, _processAndSendBuffer]);

  // ─── Stop collection early ────────────────────────────────────────────────
  const stopCollection = useCallback((reason = 'page_exit') => {
    stopTimers();

    if (isCollectingRef.current && !hasCollectedRef.current) {
      console.log(`[BUFFER COLLECTION] Stopping early (reason: ${reason}) — sending collected buffer`);
